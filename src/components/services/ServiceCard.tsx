...
export default function ServiceCard({ item }: ServiceCardProps) {
  ...
  return (
    <Link
      to={item.link}
      className={`flex flex-col bg-black group overflow-hidden ${item.gridClass} cursor-pointer focus:outline-none focus:ring-2 focus:ring-cybersky-red`}
    >
      {/* ZONE IMAGE */}
      ...
      <div className="p-8 flex flex-col grow">
        <h3 className="text-white font-bold text-xl mb-3">{t(item.title)}</h3>
        ...
        <div className="flex items-center gap-1 text-white text-sm font-medium transition-colors group-hover:text-cybersky-red w-fit">
          {t('services.card_more')} <ArrowUpRight size={16} strokeWidth={2.5} />
        </div>
      </div>
    </Link>
  );

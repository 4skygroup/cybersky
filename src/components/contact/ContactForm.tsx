...
function CallbackForm() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (res.ok) {
            setStatus("sent");
            setNom(""); setPrenom(""); setTel(""); setSujet(""); setHoraire("");
        } else {
            setStatus("error");
        }
    };
    ...
    return (
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                <Field label={t('contact.form.last_name')} placeholder="Dupont" value={nom} onChange={setNom} />
                <Field label={t('contact.form.first_name')} placeholder="Jean" value={prenom} onChange={setPrenom} />
            </div>
            ...
            <button type="submit" disabled={status === "sending"} className="mt-2 flex w-full items-center justify-center gap-2 rounded-xl bg-white py-3 text-sm font-semibold text-black transition hover:bg-white/90 active:scale-[0.98] disabled:opacity-50">
                <Phone className="w-4 h-4" />
                {status === "sending" ? t('contact.form.sending') : t('contact.form.btn_call')}
            </button>
        </form>
    );
}
...
function MessageForm() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (res.ok) {
            setStatus("sent");
            setNom(""); setPrenom(""); setEmail(""); setSujet(""); setMessage("");
        } else {
            setStatus("error");
        }
    };
    ...
    return (
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
            ...
            <button type="submit" disabled={status === "sending"} className="mt-2 flex w-full items-center justify-center gap-2 rounded-xl bg-white py-3 text-t5 font-semibold text-black transition hover:bg-white/90 active:scale-[0.98] disabled:opacity-50">
                <Mail className="w-4 h-4" />
                {status === "sending" ? t('contact.form.sending') : t('contact.form.btn_message')}
            </button>
        </form>
    );
}
...
function SelectField({ label, options, value, onChange, defaultText }: { label: string; options: string[]; value: string; onChange: (v: string) => void; defaultText: string; }) {
    return (
        <div className="flex flex-col gap-1.5">
            <label className="text-t6 font-medium uppercase tracking-widest text-white/50">{label}</label>
            <select value={value} onChange={(e) => onChange(e.target.value)} className="w-full appearance-none rounded-xl border border-white/10 bg-white/5 px-4 py-3 text-t5 text-white outline-none transition focus:border-white/30 focus:bg-white/8">
                <option value="" disabled className="bg-[#111] text-white/50">{defaultText}</option>
